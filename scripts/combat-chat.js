const MODULE_NAME = 'combat-chat'
const DISPLAY_NAME = 'Combat Chat'
...
const COMBAT_CHAT_CLASS = 'combat-chat'
...
function updateCombatTrackerStyle () {
  ...
  if (enable && activeTab === COMBAT_ID) {
    chatElement.classList.add(ACTIVE_CLASS, COMBAT_CHAT_CLASS)
    scrollChatToEnd()
  } else if (activeTab !== CHAT_ID) {
    chatElement.classList.remove(ACTIVE_CLASS)
  } else if (activeTab !== COMBAT_ID) {
    chatElement.classList.remove(COMBAT_CHAT_CLASS)
  }
